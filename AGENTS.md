# AuthRail Agent Instructions

## Product Thesis

AuthRail is an approval and execution-control layer for sensitive actions performed by AI agents.

AuthRail is not IAM, SSO, Auth0, Okta, or a generic identity provider. It does not replace an organization's existing identity, access management, authentication, or authorization systems. AuthRail sits between AI agents and sensitive execution surfaces to evaluate policy, require human approval when needed, issue execution grants, and record an immutable audit trail.

The core product flow is:

1. An AI agent proposes a sensitive action.
2. AuthRail evaluates organization policies.
3. AuthRail returns `allow`, `deny`, or `approval_required`.
4. If approval is required, a human reviews the action.
5. If approved, AuthRail executes the action or emits an execution grant.
6. Every step is recorded in an immutable audit trail.

## Product Priorities

1. Approval control for AI-agent actions.
2. Policy evaluation before execution.
3. Human review for actions that require approval.
4. Execution grants that are explicit, scoped, and auditable.
5. Immutable audit records for proposal, decision, review, approval, denial, execution, and grant issuance.
6. Clear integration points for agent frameworks, tools, and execution environments.

## Things To Avoid

- Do not position AuthRail as IAM, SSO, Auth0, Okta, or a generic identity provider.
...
- Do not make AuthRail a generic workflow tool.
...
- Keep integrations narrow: AuthRail controls whether and how an action may execute; it does not own the entire external system.

## Definition Of Done

A change is done only when:

- It preserves the product thesis that AuthRail is an approval and execution-control layer for AI agents.
- It does not present AuthRail as IAM, SSO, Auth0, Okta, or a generic identity provider.
